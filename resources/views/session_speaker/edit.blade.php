                    ...
                    >
                        @foreach($sessions as $session)
                            <option value="{{ $session->id }}"
                                {{ old('session_id', $mapping->event_session_id ?? '') == $session->id ? 'selected' : '' }}>
                                {{ $session->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- ✅ Status Dropdown -->
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                        >
                            <option value="1" {{ old('status', $mapping->status ?? 1) == 1 ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ old('status', $mapping->status ?? 1) == 0 ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
